import { Scheduler, ArrayHelper, DateHelper, Rectangle, ResourceTimeRangeStore } from '../../build/scheduler.module.js?456730';
...
StartTest(t => {
    ...
    t.it('Setting non empty dataset should correctly update UI, visible range', async t => {
        ...
        scheduler.resourceTimeRangeStore.data = [{
            id : 1, resourceId : 1, startDate : startDate, duration : 3, durationUnit : 'days', name : 'New range'
        }];
    ...
    t.it('Should be able to set resourceTimeRangeStore at runtime', async t => {
        const
            ranges1 = [
                { id : 1, resourceId : 1, startDate : startDate, duration : 4, durationUnit : 'days', name : 'Range 1', cls : 'custom' },
                { id : 2, resourceId : 1, startDate : DateHelper.add(startDate, 5, 'days'), duration : 2, durationUnit : 'days', name : 'Range 2' },
                { id : 3, resourceId : 1, startDate : DateHelper.add(startDate, 17, 'days'), duration : 4, durationUnit : 'days', name : 'Range 3' }

            ],
            ranges2 = [
                { id : 1, resourceId : 1, startDate : DateHelper.add(startDate, 1, 'days'), duration : 2, durationUnit : 'days', name : 'Range 2-1', cls : 'range-2-1' },
                { id : 2, resourceId : 1, startDate : DateHelper.add(startDate, 4, 'days'), duration : 2, durationUnit : 'days', name : 'Range 2-2', cls : 'range-2-2' },
                { id : 3, resourceId : 1, startDate : DateHelper.add(startDate, 7, 'days'), duration : 4, durationUnit : 'days', name : 'Range 2-3', cls : 'range-2-3' }
            ];

        await createScheduler(5);

        // Initial rendering
        await t.waitForSelector('.b-sch-resourcetimerange.custom');

        // Set instance of store
        ...
        t.selectorExists('.b-sch-resourcetimerange.range-2-3');
    ...
    t.it('Should not disappear on scroll with many stacked events', async t => {
        ...
        scheduler.scrollTop = 400;

        // Not waiting for selector, since that could catch the initial range and we want to make sure it is there
    ...
    t.it('Should not find artifacts from event when reusing element', async t => {
        scheduler = await t.getSchedulerAsync({
            ...
            resourceTimeRanges : [
                { id : 5, resourceId : 1, startDate : '2019-01-02T12:00', endDate : '2019-01-02T14:00', name : 'Lunch' },
                { id : 6, resourceId : 1, startDate : '2019-01-02T16:00', endDate : '2019-01-02T20:00', name : 'Lunch asdad' }
            ]
        });
    ...
    t.it('Should render events when window height increases', async t => {
        ...
        scheduler = await t.getSchedulerAsync({
            ...
            events : [
                { id : 7, resourceId : 'r6', startDate : new Date(2019, 0, 1, 12, 30), duration : 1, name : 'Important meeting' }
            ],
            resourceTimeRanges : [
                { id : 2, resourceId : 'r6', startDate : '2019-01-01T11:00', duration : 1, name : 'Lunch' }
            ]
        });
    ...
    t.it('Should not crash with repurposed elements and custom html eventRenderer', async t => {
        ...
        scheduler = await t.getSchedulerAsync({
            ...
            resources : ArrayHelper.populate(count, i => ({ id : i, name : i })),
            events    : ArrayHelper.populate(count / 2, i => ({ id : i, name : i, resourceId : i * 2, startDate : '2021-01-01T11:00', duration : 1 })),

            resourceTimeRanges : ArrayHelper.populate(count, i => ({ id : i, name : i, resourceId : i, startDate : '2021-01-01T18:00', duration : 1 })),
    ...
    t.it('Should not show html from custom eventRenderer in vertical scheduler', async t => {
        scheduler = await t.getSchedulerAsync({
            ...
            resources : ArrayHelper.populate(10, i => ({ id : i, name : `Resource ${i}` })),
            events    : [
                { id : 1, resourceId : 9, startDate : '2021-10-21T09', endDate : '2021-10-21T10' }
            ],
            resourceTimeRanges : [
                { id : 2, resourceId : 0, startDate : '2021-10-21T10', endDate : '2021-10-21T12', name : 'Range' }
            ],
