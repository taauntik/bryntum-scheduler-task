...
StartTest(t => {
  ...
  t.it('Should resize', async t => {
    ...
    t.chain({
      ...
      let transitionCount = 0; // The two drag operations should cause only two width transitions - that's at the mouseup.
      // The width changing during the drag must not be animated.

      scheduler.contentElement.addEventListener('transitionstart', ({
        target,
        propertyName
      }) => {
  ...
  t.it('DOM element of target being resized should be above any other event DOM elements', async t => {
    ...
    t.chain({
      ...
      const resizeElZIndex = window.getComputedStyle(document.querySelector(scheduler.eventSelector + '.b-sch-event-wrap-resizing'))['z-index'],
  ...
  t.it('Should add/remove b-sch-terminals-visible class to/from event inner', async t => {
    ...
    const eventInner = scheduler.getElementFromEventRecord(scheduler.eventStore.first),
  ...
  t.it('Should never derender an event while resizing', async t => {
    ...
    await t.waitFor(() => scheduler.visibleDateRange.endDate >= targetScrollDate); // Must not have been derendered
