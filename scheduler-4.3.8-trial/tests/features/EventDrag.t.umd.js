...
StartTest({
  ...
  t.it('CSS classes + styling sanity checks', async t => {
    ...
    t.chain({
      ...
      const draggedElementStyles = window.getComputedStyle(t.query(scheduler.eventSelector + '.b-dragging')[0]),
  ...
  t.it('Should abort on ESC keypress and restore dependencies', async t => {
    ...
    const eventEls = scheduler.eventStore.map(e => scheduler.getElementFromEventRecord(e)),
    ...
    // on its transform style, so we have to wait for its rectangle to settle back.
    {
      ...
      const endingEventEls = scheduler.eventStore.map(e => scheduler.getElementFromEventRecord(e)),
            endingEventElRecs = endingEventEls.map(e => Rectangle.from(e)); // Same number of elements, and all in the same place.
  ...
  t.it('Should display correct drop position with snap (horizontal)', async t => {
    ...
    }, // Grab it on the left because allowOver means that the center is occluded by the event tip
    {
      ...
      waitFor: () => {
        const draggedElementBox = Rectangle.from(t.query('.b-dragging')[0]); // The snapping to increment should mean that the dragged element won't move only 10px.
