...
StartTest({
  ...
  async function getScheduler(config = {}) {
    return await t.getSchedulerAsync(_objectSpread({
      features: {
        eventDrag: true
      }
    }, config));
  ...
  t.it('Should not deselect events on multi event drag with Ctrl', async t => {
    ...
    const [e1, e2, e3] = scheduler.eventStore,
  ...
  t.it('Should drag multi events to the same resource if unifiedDrag : true when scrolling down the dataset', async t => {
    ...
    const schedulerRectangle = Rectangle.from(scheduler.element),
    ...
    next => {
      ...
      t.isDeeply(scheduler.selectedEvents.map(e => e.id), [3, 2, 1], 'Correct selection'); // All should have been dragged to same resource
  ...
  t.it('Should drag multi events to the same resource if unifiedDrag is not set when scrolling down the dataset', async t => {
    ...
    next => {
      ...
      t.isDeeply(scheduler.selectedEvents.map(e => e.id), [3, 2, 1], 'Correct selection'); // All should have been dragged to same offset from their start row
  ...
  t.it('Multi drag, derendering second event horizontally, then bringing it back', async t => {
    ...
    const visibleEventSelector = scheduler.unreleasedEventSelector,
