...
StartTest({
  ...
  t.it('dragging outside the rendered block', async t => {
    ...
    const resources = ArrayHelper.populate(200, i => ({
      id: `r${i + 1}`,
      name: `Resource ${i + 1}`
    })),
    ...
    }, // This will kick off scrolling
    {
      ...
      waitFor: () => scheduler.rowManager.topIndex >= 100,
      timeout: 40000,
      desc: 'Scrolling'
    }, {
      moveMouseTo: document.body,
      ...
      const row = scheduler.rowManager.getRowFor(droppedOnResource),
  ...
  t.it('dragging outside the rendered block with ESC to abort', async t => {
    ...
    }, // This will kick off scrolling
    {
      ...
      endingEventElRects = endingEventEls.map(e => Rectangle.from(e)); // Same number of elements, and all in the same place.
  ...
  t.it('should work crossing TimeAxis window boundaries', async t => {
    ...
    const event = scheduler.eventStore.first; // So that the drag snaps to a day boundary making test reliable
  ...
  t.it('Should handle some events being removed after dragging to invalid drop position', async t => {
    ...
    await t.mouseUp();
    await t.waitForSelectorNotFound(scheduler.unreleasedEventSelector + ':contains(Event 2)'); // This should not crash
  ...
  t.it('Should show tooltip anchored to dragged element if it goes out of view then back in', async t => {
    ...
    await t.dragBy('.b-sch-event:contains(foo)', [50, 0]); // Drag out / in (have to drag in two steps for safari to behave as expected)
  ...
  t.it('Should navigate to next event correctly after removing assignments one by one', async t => {
    ...
    t.is(scheduler.activeAssignment.event.id, 3, '3rd event is active'); // Drag 1st event after 2nd
