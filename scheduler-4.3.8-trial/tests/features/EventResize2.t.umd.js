...
StartTest(t => {
  ...
  t.it('Should not be possible to resize event if resize is in progress', async t => {
    ...
    let resizeContext;
    scheduler.on('beforeeventresizefinalize', ({
      context
    }) => {
      context.async = true;
  ...
  t.it('should work crossing TimeAxis window boundaries', async t => {
    ...
    const event = scheduler.eventStore.first,
          jan27th = new Date(2019, 0, 27); // So that the drag snaps to a day boundary making test reliable

    scheduler.timeAxis.resolutionUnit = 'day';
    await t.mouseDown(scheduler.eventSelector, null, ['100%-2', '50%']);
    await t.moveMouseTo(scheduler.timeAxisSubGridElement, null, null, [scheduler.timeAxisSubGridElement.offsetWidth - 20, 25]); // Wait until we have resized past the Jan 27 start point.
    ...
    await t.mouseUp();
    await t.waitFor(() => !scheduler.features.eventResize.context); // Get the wrapper.

    const eventEl = scheduler.getElementFromEventRecord(event, event.resource, true); // 6th Dec 2019 is at X=900 now that we have shifted once (since the initial shift).
    ...
    t.selectorCountIs(`${scheduler.eventSelector}:not(.b-released)`, 1, 'Event element is present'); // Event endpoint dragged end into future time axis frame
