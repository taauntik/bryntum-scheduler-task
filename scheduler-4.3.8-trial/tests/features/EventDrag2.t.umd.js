...
StartTest(t => {
  ...
  t.it('should scroll correctly during drag after a previous drag/drop has caused a row height change', async t => {
    ...
    }, // It should scroll back to 0, triggering changeTotalHeight which should be ignored.
    {
      ...
      trigger: () => scrollable.scrollTo(null, 0, {
        animate: true,
        force: true
      })
    }, () => {
  ...
  t.it('Should correctly display proxy position on scrolled page', async t => {
    async function DoTest(t, constrainDragToTimeline) {
      ...
      t.chain({
        ...
        const el = document.querySelector('.b-dragging:not(.b-hidden)') || scheduler.features.eventDrag.drag.context.element,
        ...
        waitFor() {
          ...
          // into the correct original position
            ...
            const el = document.querySelector('.b-sch-event.event1'),
                  box = el.getBoundingClientRect();
  ...
  t.it('Should be possible to freely drag unconstrained event after dragging a constrained event', async t => {
    ...
    t.chain({
      ...
      by: [200, 0],
      dragOnly: true
    }, async () => t.isGreater(t.query(scheduler.eventSelector + ':contains(free)')[0].getBoundingClientRect().left, t.query(scheduler.eventSelector + ':contains(constrained)')[0].getBoundingClientRect().left, 'Free event could move along x-axis'), {
      mouseUp: null
    }, {
      waitForSelectorNotFound: '.b-dragging-event'
