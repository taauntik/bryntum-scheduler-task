...
StartTest(t => {
      ...
      for (let i = 0; i < html.length; i++) {
        ...
        t.subTest(`Checking page: ${i}`, t => {
          ...
          const event = scheduler.eventStore.find(r => {
            return DateHelper.intersectSpans(startDate, endDate, r.startDate, r.endDate) && !r.isMilestone && doc.querySelector(`[data-id="${r.resourceId}"]`);
          });
  ...
  t.it('Should export dependencies to multiple pages', async t => {
    ...
    t.chain(next => {
      ...
      await assertExport(false); // In headless mode these two exports run too fast and fail when all `it`s are running. Nothing happens
