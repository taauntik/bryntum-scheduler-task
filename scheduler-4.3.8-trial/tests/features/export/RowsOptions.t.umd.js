...
StartTest(t => {
  ...
  t.it('Should export visible rows', async t => {
    ...
    function assertContent(doc) {
      const rows = Array.from(doc.querySelectorAll('.b-grid-subgrid-normal .b-grid-row')),
  ...
  t.it('Should export visible rows (scrolled)', async t => {
    ...
    function assertContent(doc) {
      ...
      const rows = Array.from(doc.querySelectorAll('.b-grid-subgrid-locked .b-grid-row')),
            bodyContainerEl = doc.querySelector('.b-grid-body-container'),
    ...
    t.chain(async () => {
      await scheduler.scrollRowIntoView(topRecord, {
        block: 'start'
      });
      t.diag('Exporting visible rows to single page');
      await doExport(RowsRange.visible, 'singlepage', assertContent);
  ...
  t.it('Should align rows', async t => {
    ...
    t.chain({
      ...
      for (let i = 0; i < totalPages; i++) {
        ...
        const rows = Array.from(doc.querySelectorAll('.b-timeline-subgrid .b-grid-row')),
