import { AjaxHelper } from '../../build/scheduler.module.js?456730';
...
StartTest(t => {
    ...
    t.it('Should support using "on"/"un" function for registering/unregistering events', async t => {
        let tipInstanceCalled, featureInstanceCalled = 0;

        scheduler = await t.getSchedulerAsync();

        const { eventTooltip } = scheduler.features;
        ...
        // Add listener
        eventTooltip.on({ beforeShow });
        await t.moveMouseTo('.b-sch-event');
        await t.waitForSelector('.b-tooltip');
        t.ok(tipInstanceCalled, 'listener fn was called');

        // Add enable/disable listener
        eventTooltip.on({ disable });
        eventTooltip.disabled = true;
        eventTooltip.disabled = false;
        t.is(featureInstanceCalled, 1, 'feature listener was called');

        // Remove listener
        eventTooltip.un({ beforeShow });
                ...
                eventTooltip : {
                    tools : {
                        testTool : {
                            ...
                            tooltip : {
                                getHtml() {
                                    testToolTooltipShown = true;

                                    // this.up call must work.
    ...
    t.it('Should only realign once per scroll', async t => {
        ...
        // Wait twice since scroll sync takes an additional frame
