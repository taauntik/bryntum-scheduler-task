...
StartTest(t => {
  ...
  t.it('Dragging should be constrained to within the time axis', async t => {
    ...
    t.selectorExists('.b-sch-timeaxiscolumn-levels-2', 'Always outputting the correct number of header levels on header container element');
    await t.dragBy('.b-sch-range', [-4.4 * scheduler.timeAxisViewModel.tickSize, 0]); // It's been successfully constrained to within the time axis and is the same duration
  ...
  t.it('Labels and lines should be synchronized after scroll', async t => {
    ...
    t.chain({
      ...
      const rangeHeaderEl = document.querySelector('.b-grid-header-container .b-sch-range'),
  ...
  t.it('Should reuse time range element', async t => {
    await createScheduler(t);
    const range = scheduler.features.timeRanges.store.getById('lunch'),
    ...
    t.waitFor(() => element.offsetWidth === width * 2, () => {
      t.is(scheduler.features.timeRanges.getElementsByRecord(range).headerElement, element, 'Element is reused');
      t.selectorCountIs('.b-grid-headers .b-timerange-TimeRanges', 2, 'Two time ranges element in DOM');
    });
  });
  ...
  t.it('Should be able to set timeRangeStore at runtime', async t => {
    const ranges1 = [{
      id: 1,
      name: 'Range 1-1',
      startDate: '2021-08-25 08:30',
      endDate: '2021-08-25 11:30',
      cls: 'time-range-1-1'
    }, {
      id: 2,
      name: 'Range 1-2',
      startDate: '2021-08-25 12:30',
      endDate: '2021-08-25 15:30',
      cls: 'time-range-1-2'
    }],
          ranges2 = [{
      id: 1,
      name: 'Range 2-1',
      startDate: '2021-08-25 06:30',
      endDate: '2021-08-25 10:30',
      cls: 'time-range-2-1'
    }, {
      id: 3,
      name: 'Range 2-3',
      startDate: '2021-08-25 14:30',
      endDate: '2021-08-25 18:30',
      cls: 'time-range-2-3'
    }];
    scheduler = await t.getSchedulerAsync({
    ...
    t.selectorExists('.b-sch-range.time-range-1-2'); // New store instance
    ...
    t.selectorExists('.b-sch-range.time-range-2-3'); // New store config object
