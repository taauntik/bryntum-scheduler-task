...
StartTest(t => {
  ...
  const createScheduler = async (labelConfig, schedulerConfig) => {
    ...
    const labels = document.querySelectorAll('.b-sch-label'); // TODO: guess there is no guarantee that browsers deliver elements in querySelectorAll in same order, might need to come up with more reliable checking
  ...
  t.it('Editing terminating with ENTER', async t => {
    ...
    const event = scheduler.eventStore.findByField('name', 'Work')[0].data,
  ...
  t.it('Editing terminating with focusout, with blurAction default, which is cancel', async t => {
    ...
    const event = scheduler.eventStore.findByField('name', 'Work')[0].data,
          duration = event.duration;
    await t.doubleClick(`[data-event-id=${event.id}] .b-sch-label-right`);
    await t.waitFor(() => document.activeElement === scheduler.features.labels.right.editor.inputField.input); // Spin up
  ...
  t.it('Layout mode, left labels', async t => {
    ...
    t.isApproxPx(t.rect('.b-grid-row[data-id="2"]').height, 111, 'Stacked with measure'); // Shorter string, wont be stacked when measuring but still stacked when estimating
    ...
    t.isApproxPx(t.rect('.b-grid-row[data-id="2"]').height, 111, 'Stacked with estimate'); // Even shorter string, not stacked at all (also makes sure html is not part of estimate)
  ...
  t.it('Layout mode, right labels', async t => {
    ...
    t.isApproxPx(t.rect('.b-grid-row[data-id="2"]').height, 111, 'Stacked with estimate');
    scheduler.features.labels.labelLayoutMode = 'measure';
    t.isApproxPx(t.rect('.b-grid-row[data-id="2"]').height, 111, 'Stacked with measure'); // Using special string for FireFox because it has different width measuring results
