...
StartTest(t => {
  ...
  t.it('Should create an event if createValidator returns true', async t => {
    ...
    t.chain({
      ...
      const tipBox = eventDragCreate.tip.element.getBoundingClientRect(),
  ...
  if (document.scrollingElement) {
    t.it('Tooltip should align correctly', async t => {
      ...
      t.chain({
        ...
        const proxy = scheduler.features.eventDragCreate.dragging.context.element,
  ...
  t.it('Should work when the first pixel of drag results in a zero duration milestone snapped to the right edge of the time axis', async t => {
    ...
    document.scrollingElement.scrollLeft = 1000; // The mousedown date should work, and not yield null due to the page scroll
