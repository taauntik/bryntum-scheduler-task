...
StartTest(async t => {
  const {
    ...
    const failed = [],
          ...
          matches = re => [...content.matchAll(re)].map(m => m[0]); // No links to classes with #


    failed.push(...matches(new RegExp(`(href|src) *= *"#${id}.*?"`, 'gm'))); // No links to .md files with #

    failed.push(...matches(/href *= *"\S+?\.md[#\S]*?"/gm)); // Examples links should have trailing backslash
  ...
  t.it('Should have correct examples online external link', async t => {
    ...
    const link = `/examples/${product === 'schedulerpro' ? 'scheduler-pro' : product}/`,
          selector = `.b-tree-cell-inner[href="${link}"]`;
    t.is(router.getRoute('../examples'), link, 'Correct examples link'); // Check navigation tree

    await t.waitForSelector(selector);
    let el = t.global.document.querySelector(selector);
    t.is(el.getAttribute('target'), '_blank', 'Examples link is external'); // Check header tools
