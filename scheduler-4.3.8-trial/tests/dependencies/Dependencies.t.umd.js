...
StartTest(t => {
  ...
  t.it('Should not throw an exception when mouse is moved out from event which is fading out due to its removing when dependencies enabled', async t => {
    ...
    await t.waitForProjectReady(scheduler); // Store element for simulator here because event would be removed during test

    const r1 = scheduler.eventStore.getAt(1),
  ...
  t.it('Should refresh dependencies after filtering with schedule region collapsed', t => {
    ...
    t.chain(() => scheduler.subGrids.normal.collapse(), next => {
      scheduler.resourceStore.filter('name', 'Resource 1');
      next();
    }, () => scheduler.subGrids.normal.expand(), {
      waitFor: () => document.querySelectorAll('.b-sch-dependency').length === 1,
  ...
  t.it('Should create dependency when pointerout is triggered before drag has actually started', async t => {
    ...
    const box = t.rect(terminal); // t.simulator.simulateEvent(terminal, 'mousemove', { clientX : box.right + 1, clientY : box.top + 1 });

    t.simulator.simulateEvent(terminal, 'mouseout');
    t.simulator.simulateEvent(scheduler.timeAxisSubGridElement, 'mousemove', {
  ...
  t.it('Should not calculate boxes during scroll', async t => {
    ...
    for (let i = 1; i < 101; i++) {
      for (let j = 1; j < 51; j++) {
        events.push({
          id: ++id,
          resourceId: i,
          startDate: DateHelper.add(startDate, j * 7, 'd'),
          duration: 5
        });
    ...
    await t.waitForDependencies();
    const boxSpy = t.spyOn(scheduler.features.dependencies, 'getBox').and.callThrough(),
