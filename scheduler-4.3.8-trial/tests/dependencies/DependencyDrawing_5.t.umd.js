...
StartTest(t => {
    ...
    for (let i = 0; i < 5; i++) {
      await t.describe(`Checking zoom level ${zoomLevel - i - 1}`, async t => {
        ...
        targetDeps.forEach(dependency => {
          const depEl = document.querySelector(`polyline[depId="${dependency.id}"]`),
                points = depEl.points,
        ...
        const depEl = document.querySelector(`polyline[depId="${dependency3.id}"]`),
              points = depEl.points;
        t.isGreaterOrEqual(points[points.length - 1].x - points[points.length - 2].x, scheduler.features.dependencies.pathFinder.startArrowMargin, 'Last line segment is greater than marker size');
      });
    }
  });
  t.it('Should not render marker if it does not fit between events (vertically)', async t => {
    ...
    const dependency = scheduler.dependencyStore.first,
  ...
  t.it('Dependency line should align with inner element', async t => {
    ...
    await t.waitForSelector('.b-sch-dependency');
    const [dep1, dep2] = scheduler.dependencyStore.getRange(),
  ...
  t.it('Should render cached dependencies after zoom in', async t => {
    ...
    await t.waitForSelector('.b-sch-dependency'); // Scroll to the event 3 to trigger dependency rendering

    await scheduler.scrollEventIntoView(scheduler.eventStore.getById(3), {
      block: 'center'
    });
    await t.waitForSelector('[depId="2"]'); // Scroll back to the 0
    ...
    await t.waitForEvent(scheduler, 'horizontalScroll'); // Assert first dependency which should be in view because we maintain center date during zoom

    t.assertDependency(scheduler, scheduler.dependencyStore.first); // Scroll to the next event, dependency should appear

    await Promise.all([scheduler.scrollEventIntoView(scheduler.eventStore.getById(3), {
      block: 'center'
