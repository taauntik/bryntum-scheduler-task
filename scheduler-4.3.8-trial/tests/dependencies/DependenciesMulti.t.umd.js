...
StartTest(t => {
  ...
  async function assertDepEndToStart(t, from, to, flip = false) {
    ...
    const depLine = t.getSVGBox(document.querySelector(depSelector)),
  ...
  t.it('Event CRUD', async t => {
    ...
    t.chain(next => {
      ...
      await assertDepNotFound(t, 'a6', 'a4'); // Add not covered since it needs an assignment, tested under Assignment CRUD
    });
  });
  ...
  t.it('Resource CRUD', async t => {
    ...
    t.chain( // Add
    {
      ...
      trigger: () => scheduler.resourceStore.insert(3, {
        id: 'r20'
      })
    }, async () => assertAllDeps(t), // Remove empty resource in the middle
    {
      waitForEvent: [scheduler, 'dependenciesDrawn'],
      trigger: () => scheduler.resourceStore.remove('r3')
    }, async () => assertAllDeps(t), // Remove resource with events
    {
      waitForEvent: [scheduler, 'dependenciesDrawn'],
      trigger: () => {
  ...
  t.it('Assignment CRUD', async t => {
    ...
    t.chain(next => {
      ...
      await assertDepEndToStart(t, 'a1', 'a7'); // Remove
  ...
  t.it('Dependency CRUD', async t => {
    ...
    {
      waitForEvent: [scheduler, 'dependenciesDrawn'],
      trigger: () => scheduler.dependencyStore.add({
        id: 'd2',
        from: 'e3',
        to: 'e2'
      })
    }, async () => await assertAllDeps(t), // Remove
    {
      waitForEvent: [scheduler, 'dependenciesDrawn'],
    ...
    }, // Update
    {
      ...
      await assertDepNotFound(t, 'a6', 'a2');
      await assertDepNotFound(t, 'a6', 'a3');
      await assertDepNotFound(t, 'a6', 'a4'); // // Remove all
  ...
  t.it('Should draw dependencies for new assignment', async t => {
    ...
    t.chain({
      ...
      waitFor: () => t.query(`polyline[toId=${scheduler.assignmentStore.last.id}]`),
      desc: 'New line drawn'
    });
  });
});