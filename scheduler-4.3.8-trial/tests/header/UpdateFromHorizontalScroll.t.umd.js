...
StartTest(t => {
  ...
  t.it('Time axis should render header elements correct when change width and scroll with animation', async t => {
    ...
    await t.waitForElementTop('.b-sch-header-row-1 .b-sch-header-timeaxis-cell[data-tick-index=0]'); // Change width

    scheduler.width = 400; // Reproducible only when animation is enabled

    await scheduler.scrollHorizontallyTo(1000, true);
    await scheduler.scrollHorizontallyTo(0, true); // Check presence of first cells in timeaxis header
  ...
  t.it('Time axis should render header elements correct when grids are partnered', async t => {
    ...
    await t.waitForElementTop('.scheduler2 .b-sch-header-row-1 .b-sch-header-timeaxis-cell[data-tick-index=0]'); // Reproducible only when animation is enabled

    await scheduler.scrollHorizontallyTo(1000, true); // Check presence of last cells in timeaxis header of the first scheduler
    ...
    await t.waitForElementTop('.scheduler1 .b-sch-header-row-1 .b-sch-header-timeaxis-cell[data-tick-index=27] span:contains(30)'); // Check presence of last cells in timeaxis header of the second scheduler
