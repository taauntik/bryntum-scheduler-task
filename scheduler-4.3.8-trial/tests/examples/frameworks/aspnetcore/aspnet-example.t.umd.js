...
StartTest(async t => {
  ...
  t.it('Testing add/update/remove resource/event', async t => {
    try {
      const [resource] = scheduler.resourceStore.add({
        Name: 'New resource'
      }),
    ...
    try {
      ...
      scheduler.crudManager.on({
        beforeSyncApply({
          ...
          t.isDeeply(response.events.rows, newEvents.map((record, index) => {
            return {
              $PhantomId: record.id,
      ...
      const [newResource] = scheduler.resourceStore.add({
        Name: 'foo'
      });
      ...
      scheduler.crudManager.on({
        beforeSyncApply({
          ...
          t.isDeeply(response.events.rows, newEvents.map(record => {
            return {
              Id: record.id,
              ResourceId: newResourceId
            };
      ...
      await scheduler.crudManager.sync();
      t.isDeeply(newResource.events.map(e => e.id), newEvents.map(e => e.id), 'Resource events are ok');
