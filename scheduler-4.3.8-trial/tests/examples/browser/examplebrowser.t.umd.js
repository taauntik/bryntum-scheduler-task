...
StartTest(t => {
  ...
  let document;
  t.setWindowSize(1024, 768); // Should not have initial header animations
  ...
  t.it('Should handle non existing theme ids gracefully', t => {
    let locationSearch; // old themes (plus extra one imitating just a bad value) mapping
  ...
  t.it('Example browser links should have trailing backslash', t => {
    const failed = []; // Examples links should have trailing backslash
