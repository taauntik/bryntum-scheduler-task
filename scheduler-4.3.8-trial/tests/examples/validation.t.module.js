StartTest(t => {
    ...
    // So that we can t.iit certain tests
    ...
    t.it('Should not fail on dbl click to create event', t => {
        t.chain(
            async() => scheduler.timeAxisSubGrid.scrollable.x = 0,
            { dblClick : '[data-region="normal"] .b-grid-row', offset : [10, '50%'], desc : 'Create new event with dbl click' }
        );
    });
