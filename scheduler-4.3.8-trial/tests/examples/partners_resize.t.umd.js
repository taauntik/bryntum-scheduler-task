...
StartTest(t => {
  ...
  t.it('Width + collapsed state should be synced when changed programmatically', t => {
    const sameWidth = () => t.samePx(t.rect(topScheduler.subGrids.locked.element).width, t.rect(bottomScheduler.subGrids.locked.element).width),
          checkWidth = width => [next => {
      ...
      waitFor: () => bottomScheduler.subGrids.locked.element.offsetWidth === width,
      desc: `Bottom locked grid has correct width ${width}`
    }];
    ...
    t.chain( // Wait for both schedulers to render events
    {
    ...
    }, checkWidth(160), checkWidth(140), async () => topScheduler.subGrids.locked.collapse(), {
      waitFor: () => bottomScheduler.subGrids.locked.collapsed === true,
      ...
      waitFor: () => bottomScheduler.subGrids.locked.collapsed === false,
  ...
  t.it('Width + collapsed state should be synced using splitters', async t => {
    const checkWidth = width => [{
      waitFor: () => topScheduler.subGrids.locked.element.offsetWidth === width,
      desc: `Top locked grid has correct width ${width}`
    }, {
      waitFor: () => bottomScheduler.subGrids.locked.element.offsetWidth === width,
      desc: `Bottom locked grid has correct width ${width}`
    }],
          checkCollapse = collapsed => [{
      waitFor: () => topScheduler.subGrids.locked.collapsed === collapsed,
      desc: `Top locked grid is ${collapsed ? '' : 'not'} collapsed`
    }, {
      waitFor: () => bottomScheduler.subGrids.locked.collapsed === collapsed,
      desc: `Bottom locked grid is ${collapsed ? '' : 'not'} collapsed`
