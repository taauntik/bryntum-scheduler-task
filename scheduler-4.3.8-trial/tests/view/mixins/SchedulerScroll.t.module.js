import { DomHelper, ArrayHelper, Scheduler } from '../../../build/scheduler.module.js?456730';
...
StartTest(t => {
    ...
    t.it('EventStore should fire loadDateRange events', async t => {
        ...
        t.wontFire(scheduler, 'timelineviewportresize');

        // Wait for extraneous events to fire.
        ...
        // Only one TimeAxisViewModel update event during setup.
        t.is(tavmUpdateCount, 1);

        // The initial range centered on 2020-01-01T12:00
        ...
        // We must wait for the bug to manifest. The styles are changed on a timer.
        // If we do not wait, the test runs to the end without registering that a
        // timelineviewportresize event fires.
        // https://github.com/bryntum/support/issues/3715
        await t.waitFor(200);

        // Only one more TimeAxisViewModel update event in response to the scroll.
    ...
    t.it('infiniteScroll should scroll back to center after snapping to right edge', async t => {
        ...
        const centerPosition = element.scrollLeft;
    ...
    t.it('Should have correct visibleDateRange in horizontalScroll listeners', async t => {
        ...
        scheduler.on('horizontalScroll', ({ subGrid }) => {
