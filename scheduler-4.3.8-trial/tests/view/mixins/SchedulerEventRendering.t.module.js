import { Scheduler, CSSHelper, Rectangle } from '../../../build/scheduler.module.js?456730';
...
StartTest(t => {
    ...
    t.it('Should wrap string config to object internally', async t => {
        ...
        t.isDeeply(scheduler.internalEventLayout, { type : 'pack' }, 'internalEventLayout is "pack"');

        scheduler.eventLayout = 'stack';

        t.is(scheduler.eventLayout, 'stack', 'eventLayout is "stack"');
        t.isDeeply(scheduler.internalEventLayout, { type : 'stack' }, 'internalEventLayout is "stack"');

        scheduler.eventLayout = 'none';

        t.is(scheduler.eventLayout, 'none', 'eventLayout is "none"');
        t.isDeeply(scheduler.internalEventLayout, { type : 'none' }, 'internalEventLayout is "none"');

        scheduler.eventLayout = { type : 'stack' };
