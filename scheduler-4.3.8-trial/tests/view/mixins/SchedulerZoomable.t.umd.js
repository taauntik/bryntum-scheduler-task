...
StartTest(t => {
  ...
  t.it('Should zoom to span', t => {
    ...
    t.chain({
      ...
      t.waitFor(() => scheduler.scrollLeft > 0, () => {
  ...
  t.it('Should respect leftMargin / rightMargin configs provided to zoomToFit', async t => {
    ...
    t.chain({
      ...
      const startDate = scheduler.getDateFromCoordinate(scheduler.scrollLeft + 60),
  ...
  t.it('Should only fire one loadDateRange event on viewpreset change', async t => {
    ...
    function generateEvents() {
      const result = []; // Loop through every day for a year surrounding today.
    ...
    scheduler.zoomIn(); // We need to wait for the bug condition which was more asynchronous loadDateRange events
    // arriving in an async timeframe.

    await t.waitFor(100);
    t.is(loadDateRangeCount, 1); // Scroll range may vary a little due to floor/ceil of end points to different tick bounds.
    ...
    await t.waitFor(100);
    t.is(loadDateRangeCount, 2); // Scroll range may vary a little due to floor/ceil of end points to different tick bounds.
    ...
    await t.waitFor(100);
    t.is(loadDateRangeCount, 3); // Scroll range may vary a little due to floor/ceil of end points to different tick bounds.
    ...
    await t.waitFor(100);
    t.is(loadDateRangeCount, 4); // Scroll range may vary a little due to floor/ceil of end points to different tick bounds.
    ...
    await t.waitFor(100);
    t.is(loadDateRangeCount, 5); // Scroll range may vary a little due to floor/ceil of end points to different tick bounds.

    t.isApprox(scheduler.timelineScroller.scrollWidth, scrollWidth, 100);
    scrollWidth = scheduler.timelineScroller.scrollWidth;
    scheduler.zoomOut(); // We need to wait for the bug condition which was more asynchronous loadDateRange events
    // arriving in an async timeframe.

    await t.waitFor(100);
    t.is(loadDateRangeCount, 6); // Scroll range may vary a little due to floor/ceil of end points to different tick bounds.
    ...
    await t.waitFor(100);
    t.is(loadDateRangeCount, 7); // Scroll range may vary a little due to floor/ceil of end points to different tick bounds.
    ...
    await t.waitFor(100);
    t.is(loadDateRangeCount, 8); // Scroll range may vary a little due to floor/ceil of end points to different tick bounds.
    ...
    await t.waitFor(100);
    t.is(loadDateRangeCount, 9); // Scroll range may vary a little due to floor/ceil of end points to different tick bounds.
