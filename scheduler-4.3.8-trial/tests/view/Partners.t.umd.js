...
StartTest(t => {
  ...
  t.beforeEach(() => {
    timeAxisSpy === null || timeAxisSpy === void 0 ? void 0 : timeAxisSpy.reset();
    timeAxisVMSpy === null || timeAxisVMSpy === void 0 ? void 0 : timeAxisVMSpy.reset();
  ...
  t.it('Scroll should be synced', async t => {
    ...
    t.chain(async () => {
      ...
      await secondScheduler.subGrids.normal.scrollable.await('scrollEnd', {
        checkLog: false
      });
    }, {
      ...
      await firstScheduler.subGrids.normal.header.scrollable.await('scrollEnd', {
        checkLog: false
      });
    }, {
      ...
      await firstScheduler.subGrids.normal.scrollable.await('scroll', {
        checkLog: false
      });
    });
  });
  ...
  if (DomHelper.scrollBarWidth) {
    ...
    t.it('Should keep scrollbars in sync', async t => {
      ...
      }, config); // Each must know about the other

      t.is(firstScheduler.partners[0], secondScheduler);
      t.is(secondScheduler.partners[0], firstScheduler);
      const firstHeaderEl = firstScheduler.timeAxisSubGrid.header.element,
      ...
      const overflowingWidth = secondTimeAxisElement.offsetWidth; // All time axes are the same width
      ...
      t.is(firstTimeAxisElement.offsetWidth, secondTimeAxisElement.offsetWidth); // Change second one to not overflowing

      secondScheduler.resourceStore.remove(secondScheduler.resourceStore.last); // Wait for processing to have made adjustments

      await t.waitFor(() => {
        return firstTimeAxisElement.offsetWidth > overflowingWidth && firstHeaderEl.offsetWidth > overflowingWidth;
      }); // Now there's no overflow, width has increased

      t.isGreater(firstTimeAxisElement.offsetWidth, overflowingWidth);
      t.isGreater(firstHeaderEl.offsetWidth, overflowingWidth); // All time axes are the same width

      t.is(firstHeaderEl.offsetWidth, secondTimeAxisElement.offsetWidth);
      t.is(secondHeaderEl.offsetWidth, firstTimeAxisElement.offsetWidth);
      t.is(firstTimeAxisElement.offsetWidth, secondTimeAxisElement.offsetWidth); // Make second one overflow again

      secondScheduler.height -= 75; // Wait for processing to have made adjustments

      await t.waitFor(() => {
        return firstTimeAxisElement.offsetWidth === overflowingWidth && firstHeaderEl.offsetWidth === overflowingWidth && secondTimeAxisElement.offsetWidth === overflowingWidth && secondHeaderEl.offsetWidth === overflowingWidth;
