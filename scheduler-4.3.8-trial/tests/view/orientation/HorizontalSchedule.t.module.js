import { ArrayHelper, DomHelper, EventModel, DomClassList } from '../../../build/scheduler.module.js?456730';
...
StartTest(t => {
    ...
    async function doScrollAndAssert(t, scheduler, i) {
        const
            { scrollable, rowHeight } = scheduler,
        ...
        const
            totalSize = Math.floor(
                (
                    // Take no more than buffer size pixels from the top
                    Math.min(scrollable.y, bufferSize) +
                    // add visible area height
                    scrollable.clientHeight +
                    // Take no more than buffer size pixels from the bottom
                    Math.min(scrollable.scrollHeight - scrollable.y - scrollable.clientHeight, bufferSize)
                ) / rowHeight),
    ...
    async function scrollVerticallyAndAssertEvents(t, scheduler, up = false) {
        ...
        // Scroll by random delta within range (0; bufferSize * 3)
        if (!up) {
    ...
    t.it('Should not render far out of view events for multiple resource rows', async t => {
        ...
        for (let i = 0; i < 1000; i++) {
            ...
            if (i % 100 === 0) {
                resources.push({ id : i / 100 + 1, name : `Resource ${i / 100 + 1}` });
            }
        }
