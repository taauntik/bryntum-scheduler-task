...
StartTest(t => {
  ...
  async function setup(config = {}) {
    scheduler = new Scheduler(_objectSpread({
  ...
  t.it('Converting a leaf node to a parent should only rerender the affected row', async t => {
    await setup();
    const aa = scheduler.resourceStore.getAt(1); // Converting a resource to a parent only renders that row
  ...
  t.it('Should not add whitespace at bottom when expanding parent', async t => {
    ...
    const maxScroll = 400,
          rowHeight = t.rect('.b-grid-cell').height; // Wait for Scheduler to be full height in FireFox

    await t.waitFor(() => t.samePx(scheduler.scrollable.maxY, maxScroll));
    t.pass('Correct initial scroll height');
    await scheduler.expand(1); // Expect two more rows to be reachable using scroll
