...
StartTest(t => {
  ...
  t.it('Should not find arrays / objects having member count growing with amount of operations performed', async t => {
    ...
    const maxFails = 10;
    let count = 0; // Look for arrays or objects with members growing in size

    for (const bryntumInstance of instances) {
      for (const name in bryntumInstance) {
        const value = bryntumInstance[name],
              skip = {
          configDone: 1
        },
  ...
  t.it('Should not find references to old replaced project / store instances', async t => {
    ...
    await t.click('.b-popup-close'); // Replace project, no references to initial project should remain
    ...
    await t.waitFor(() => drawCount >= 2); // Look for references to old project / stores in Scheduler
  ...
  t.it('Should not retain references to old records after reloading store / crudManager', async t => {
    ...
    await scheduler.crudManager.load(); // Look for references to old project / stores in Scheduler
  ...
  t.it('Should destroy class instances created by Scheduler (THIS t.it SHOULD RUN LAST)', async t => {
    ...
    await scheduler.project.commitAsync(); // destroy shared tooltip
