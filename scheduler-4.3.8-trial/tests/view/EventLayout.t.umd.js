...
StartTest(t => {
  ...
  t.it('Number of rendered events should remain stable during scrolling', async t => {
    ...
    await t.waitForProjectReady();
    let startEventCount; // Scroll down enough to fill the vertical render buffer, if we do not the event count wont match as we scroll
    // further down
    ...
    t.chain( // Wait for events to have been drawn after scroll
    {
      ...
      await scheduler.scrollRowIntoView(`r${200 - scheduler.leadingBufferSize}`); // Event count must be stable
  ...
  t.it('Should recalculate scroll height when stacking changes the height of a resource row', async t => {
    ...
    async () => {
      const events = scheduler.eventStore.getRange(),
            toAdd = [];
  ...
  t.it('Should recalculate scroll height and move subsequent rows downwards when stacking changes the height of a single resource row', async t => {
    ...
    async () => {
      ...
      await t.waitForProjectReady();
      const heightIncrement = row40.height - oldRow40Height; // Check all following rows have been bumped down just right
