import { Scheduler, DomHelper, ArrayHelper, DateHelper } from '../../build/scheduler.module.js?456730';
...
StartTest(t => {
    ...
    t.it('Events should not disappear on assigning new resources with same ids', async t => {
        ...
        const
            resources = [...Array(5).keys()].map(i => {
                return { id : `r${i}`, name : `Test ${i}` };
            }),
        ...
        scheduler.scrollable.element.scrollTop = 1;

        // Let the events to disappear in case of problems
        await t.waitFor(1);

        // The event must be visible at this point
