...
StartTest(t => {
  const events = [],
        eventEls = [];
  let scheduler, eventStore, assignments, selectionChangeSpy, assignmentSelectionChangeSpy;
  ...
  t.it('Should clear selection classes on released elements', async t => {
    ...
    t.chain({
      ...
      await scheduler.scrollToDate(new Date(2011, 0, 16), {
        block: 'start'
      });
      ...
      await scheduler.scrollToDate(new Date(2011, 0, 2), {
        block: 'start'
      });
      await t.waitForSelectorNotFound(`${scheduler.unreleasedEventSelector} .b-sch-event-selected`);
