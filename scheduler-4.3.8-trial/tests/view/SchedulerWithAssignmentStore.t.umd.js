...
StartTest(t => {
  ...
  const monday = new Date(2018, 4, 7),
  ...
  t.it('Reassigning a multi assigned event should update element', async t => {
    await createMultiScheduler();
    const initialTop = document.querySelector('[data-event-id="1"]').getBoundingClientRect().top; // Reassign to new resource
  ...
  t.it('Deleting events should work', async t => {
    await createScheduler(); // Need to click since bug was in EventNavigation & EventSelection
  ...
  t.it('Delete event with keyboard (multi-assign scheduler)', async t => {
    ...
    const feature = scheduler.features.eventEdit; // Focus should stay within the Scheduler by moving to the next closest event.

    t.chain(async () => t.is(assignmentStore.count, 3, 'Correct initial assignment count'), // Focus the first event assignment
    {
  ...
  t.it('Delete event with keyboard then mouse (multi-assign scheduler)', async t => {
    ...
    const {
      eventStore,
      assignmentStore
    } = scheduler; // Focus should stay within the Scheduler by moving to the next closest event.
