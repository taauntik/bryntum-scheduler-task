...
StartTest(t => {
    ...
    t.it('Should warn when load response format is incorrect if validateResponse is true', async t => {
        ...
        t.mockUrl('foo', {
            responseText : JSON.stringify({
                ...
                myEvents : { // <---------- wrong "events" section name
                    rows : [{
                        id   : 1,
                        name : 'Event'
                    }]
        ...
        t.like(consoleMessages.shift(),
            buildLinesRegExp([
                'CrudManager load response error(s):',
        ...
        t.mockUrl('foo', {
            responseText : JSON.stringify({
                ...
                myResources : { // <---------- wrong "resources" section name
                    rows : [{
                        id   : 1,
                        name : 'Man'
                    }]
        ...
        t.like(consoleMessages.shift(),
            buildLinesRegExp([
                'CrudManager load response error(s):',
                '- No "resources" store section found. It should contain the store data.',
                'Please adjust your response to look like this:',
                '{',
                '    "resources": {',
                '        "rows": [',
    ...
    t.it('Should warn when sync response format is incorrect', async t => {
        ...
        t.like(consoleMessages.shift(),
            buildLinesRegExp([
                ...
                '- "events" store section should be an Object.',
                'Please adjust your response to look like this:',
                '{',
                '    "events": {',
                '        ,,,',
    ...
    t.it('Should warn when sync response format is incorrect and supportShortSyncResponse is false', async t => {
        ...
        t.like(
            ...
            buildLinesRegExp([
                'CrudManager sync response error(s):',
                '- "events" store "rows" section should mention added record(s) #XXX sent in the request. ' +
    ...
    t.it('Should warn on multiple errors when sync response format is incorrect and supportShortSyncResponse is false', async t => {
        ...
        t.like(
            ...
            buildLinesRegExp([
                ...
                '    }',
                '}',
                'Note: Please consider enabling "supportShortSyncResponse" option to allow less detailed sync responses ' +
                '(https://bryntum.com/docs/scheduler/api/Scheduler/crud/AbstractCrudManagerMixin#config-supportShortSyncResponse)',
                'Note: To disable this validation please set the "validateResponse" config to false'
            ]),
            'Correct warn message shown.'
        );
