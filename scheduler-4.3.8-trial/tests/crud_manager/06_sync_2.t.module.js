import { CrudManager, ProjectModel, Store } from '../../build/scheduler.module.js?456730';
...
StartTest(t => {
    ...
    t.it('Fields with complex mapping should be updated properly', async t => {
        ...
        t.mockUrl('sync', {
            responseText : JSON.stringify({
                ...
                resources : {
                    rows : [
                        { $PhantomId : newResource.id, Id : 11 },
                        { Id : 1, Foo : { Bar : 'Chris' } }
                    ]
                }
            })
    ...
    t.it('Should properly unbind listeners on removing crud stores', async t => {
        ...
        const { resourceStore, eventStore } = project;
        ...
        t.mockUrl('sync', {
            responseText : JSON.stringify({
                ...
                events  : {
                    rows : [
                        { id : 122, $PhantomId : 22, eventDisplayName : 'First event name' }
                    ]
                }
            })
        });
