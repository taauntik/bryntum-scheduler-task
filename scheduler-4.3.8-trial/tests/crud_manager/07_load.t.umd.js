...
StartTest(t => {
  ...
  let sent = 0; // dummy transport implementation
  ...
  t.it('Should cancel load request', async t => {
    ...
    let cancelled = false; // Call first load to scheduler request, do not wait for it to end
  ...
  t.it('Should handle server responding garbage / 500 error', async t => {
    ...
    await t.waitFor(() => !cm.isCrudManagerLoading); // Only run in debug env
