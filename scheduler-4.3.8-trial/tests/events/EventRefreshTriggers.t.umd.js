...
StartTest(t => {
  let scheduler, eventStore, resourceStore; // async beforeEach doesn't work in umd
  ...
  t.it('Should repaint correctly if resource store is filtered', async t => {
    ...
    await t.waitForProjectReady(scheduler);
    t.waitFor(() => {
      const eventTopAfterAdd = Rectangle.from(document.body.querySelector('.b-sch-event.bar')).y; // Second event will have moved upwards
  ...
  t.it('Should repaint when removing a resource', async t => {
    const eventTopBefore = document.body.querySelector('[data-event-id="2"]').getBoundingClientRect().top; // The resourceModel.unassignAll() causes a layout as well as the following remove.
  ...
  t.it('Adding events should increase row height', async t => {
    const heightBefore = scheduler.getRow(0).height,
          countBefore = document.querySelectorAll(scheduler.unreleasedEventSelector).length; // Add will rerender all events below
  ...
  t.it('Should revert + repaint events after calling revertChanges', async t => {
    await t.waitForProjectReady();
    const eventRecord = scheduler.eventStore.getById(5),
