...
StartTest(t => {
    ...
    t.it('Should commit new event and assignment', async t => {
        ...
        const { eventStore, resourceStore, assignmentStore } = project;

        const [resource] = resourceStore.add({ name : 'New resource' });
        const [event] = eventStore.add({ name : 'New' });
        ...
        t.is(resourceStore.added.values[0], resource, 'Resource instance is ok');

        // When not using crud manager, assignment store added bag doesn't include new assignment which is by design -
