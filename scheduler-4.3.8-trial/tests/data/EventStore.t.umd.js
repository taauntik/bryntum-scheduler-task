...
StartTest(t => {
  ...
  t.it('isDateRangeAvailable with multi-assignments should work', async t => {
    ...
    const {
      ...
      assignmentStore
          ...
          assignment1 = assignmentStore.getById('a11'),
          // For example when you drag an event element which belongs to the first resource
    assignment2 = assignmentStore.getById('a12'),
          // For example when you drag an event element which belongs to the last resource
    resource = project.resourceStore.getById('r2'),
          // For example when you drag an event element to the second resource row
    firstEvent = eventStore.first,
          lastEvent = eventStore.last;
    await project.commitAsync();
  ...
  t.it('Data should be ready after loadDataAsync()', async t => {
    ...
    await eventStore.loadDataAsync([{
      startDate: '2020-09-11',
      duration: 3
    }]);
    t.is(eventStore.first.endDate, new Date(2020, 8, 14), 'Calculations performed');
  ...
  t.it('reapplyFilterOnAdd : false should work with getEvents API', async t => {
    ...
    const event3 = eventStore.createRecord({
      id: 3,
      name: 'Event 3',
      category: 'A',
  ...
  t.it('getEvents using dateMap : true', async t => {
    ...
    t.is(eventMap.size, 2); // Date range collected correctly

    t.isDeeply(eventMap.get('2010-12-21').map(e => e.id), [1, 2]);
    t.isDeeply(eventMap.get('2010-12-22').map(e => e.id), [3]); // Filter out evenbts 1 and 2
    ...
    t.isDeeply(eventMap.get('2010-12-22').map(e => e.id), [3]); // Unfilter
    ...
    t.is(eventMap.size, 2); // Clearing the filter returns all dates

    t.isDeeply(eventMap.get('2010-12-21').map(e => e.id), [1, 2]);
    t.isDeeply(eventMap.get('2010-12-22').map(e => e.id), [3]); // Deleting events one and two should mean no cell returned for that date
