...
StartTest(t => {
  ...
  t.it('Should be able to get/set inlineData on project', t => {
    ...
    const inlineData = scheduler.project.inlineData; // Check data
  ...
  t.it('Should relay store changes', async t => {
    ...
    t.isDeeply(changeData, {
      ...
      action: 'update'
    }, 'Correct args for EventStore update');
    ...
    t.isDeeply(changeData, {
      store: project.eventStore,
      action: 'add'
    }, 'Correct args for EventStore add'); //
    // RESOURCESTORE
    //
    ...
    await project.commitAsync();
    t.isDeeply(changeData, {
      store: project.resourceTimeRangeStore,
      action: 'update'
    }, 'Correct args for ResourceTimeRangeStore update');
  }); // https://github.com/bryntum/support/issues/3281
