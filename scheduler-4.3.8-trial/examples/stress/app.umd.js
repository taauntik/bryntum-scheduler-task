...
class ColleagueSimulator {
  constructor(config) {
    ...
    this.scheduler.element.addEventListener('animationend', ({
      target,
      animationName
    }) => {
  ...
  getRandomResourceName() {
    ...
    return StringHelper.encodeHtml(((_this$resourceStore$g = this.resourceStore.getAt(Math.round(Math.random() * this.resourceStore.count))) === null || _this$resourceStore$g === void 0 ? void 0 : _this$resourceStore$g.name) || 'Mystery man');
  }
