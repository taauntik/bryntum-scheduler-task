...
class ExamplesApp {
  constructor() {
    ...
    document.getElementById('title').firstElementChild.innerHTML = `${me.product.fullName} ${version}`;
    GlobalEvents.on({
      theme() {
        if (me.rendered) {
          me.refresh();
  ...
  getDomConfig() {
    const // Use the getter which relies on DomHelper.themeInfo getter which creates a DOM element and extracts theme name from it,
  ...
  render() {
    ...
    Widget.attachTooltip(me.examplesContainerEl, {
      ...
      getHtml: async ({
        ...
        const url = `${linkNode.getAttribute('href') || linkNode.dataset.linkUrl}/app.config.json`; // Cancel all ongoing ajax loads (except for the URL we are interested in)
