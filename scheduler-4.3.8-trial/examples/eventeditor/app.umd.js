...
const resourceComboConfig = {
  ...
  listeners: {
    ...
    focusOut() {
      ...
      this.store.filter(); // Now we leave the store in its pristine state, the Scheduler UI can respond
...
const scheduler = new Scheduler({
  ...
  listeners: {
    eventEditBeforeSetRecord({
      ...
      editor.title = `Edit ${eventRecord.eventType || ''}`; // Only CEO and CTO roles are allowed to play golf...
    ...
    selectionChange({
      ...
      if (mode === 'row' && selected !== null && selected !== void 0 && selected.length) {
