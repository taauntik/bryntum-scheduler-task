...
const // Normalizes agenda items on load by converting string dates to actual dates and by calculating start and end offsets
...
normalizeAgendaItems = eventData => {
  ...
  (_eventData$agenda = eventData.agenda) === null || _eventData$agenda === void 0 ? void 0 : _eventData$agenda.forEach(nestedEvent => {
...
refreshAgendaDates = eventRecord => {
  ...
  (_eventRecord$agenda = eventRecord.agenda) === null || _eventRecord$agenda === void 0 ? void 0 : _eventRecord$agenda.forEach(nestedEvent => {
...
const scheduler = new Scheduler({
  ...
  eventRenderer({
    ...
    const // Note that during a resize, we have to use `eventRecord.get('startDate')` to get the current value.
    ...
    dateToPx = date => this.getCoordinateFromDate(date) - renderData.left; // Calculate coordinates for all nested events and put in an array passed on to eventBodyTemplate
